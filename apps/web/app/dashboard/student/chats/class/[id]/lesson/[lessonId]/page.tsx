...
type LessonItem = { id: string; isActive: boolean; lesson: { id: string; title: string } };

export default function StudentClassLessonChatPage({ params }: { params: Promise<{ id: string; lessonId: string }> }) {
  ...
  const currentLesson = ((lessons || []) as LessonItem[]).find((l: LessonItem) => l.lesson.id === lessonId) || null;
              ...
              ((lessons || []) as LessonItem[]).map((item: LessonItem) => (
                <Link
                  ...
                  className={`block p-4 border-b border-gray-100 hover:bg-gray-50 transition-colors ${
                    lessonId === item.lesson.id ? "bg-indigo-50 border-r-2 border-r-indigo-500" : ""
                  }`}
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <h3 className="text-sm font-semibold text-gray-900 truncate">{item.lesson.title}</h3>
                        {item.isActive && <span className="text-xs font-medium text-green-600">Active</span>}
                      </div>
